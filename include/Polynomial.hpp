...
struct Polynomial: public std::vector<Type> {
	...
	Complex<Type> operator()(Complex<Type> const &x) const {
		Polynomial const &a = *this;
		Complex<Type> f = Complex<Type>(Type(a[degree()]));
		for (int n = degree() - 1; n >= 0; n--) {
			f = x * f + a[n];
		}
		return f;
...
Polynomial<Type> polynomialDiscreteAntiDerivative(Polynomial<Type> const &P, size_t N) {
	...
	for (int p = 0; p <= P.degree(); p++) {
		Q[p + 1] += P[p] / Type(p + 1);
		for (int j = 0; j < p; j++) {
			Q[p - j] -= P[p] * binco(p, j) * std::riemann_zeta(-j);
		}
	}
	return Q;
}
...
template<typename Type>
Complex<Type> polynomialFindComplexRoot(Polynomial<Type> const &F, Complex<Type> guess = Complex<Type>(0.5, 0.1)) { constexpr
	int maxIters = 1000;
...
template<typename Type>
std::vector<Complex<Type>> polynomialFindAllRoots(Polynomial<Type> const &F) {
	if (F.degree() == 0) {
		return std::vector<Complex<Type>>();
	} else if (F.degree() == 1) {
		std::vector<Complex<Type>> root(1, Complex<Type>(-F[0] / F[1]));
		return root;
	} else {
		static const Type tinyEps = Type(1000.0 * std::numeric_limits<double>::epsilon());
		std::vector<Complex<Type>> roots;
		...
		Complex<Type> z = radius * exp(Complex<Type>(0, 1) * theta);
...
template<typename Type>
std::string toString(Polynomial<Type> const &P, char var) {
    std::ostringstream oss;
    bool firstTerm = true;
    for (int i = P.degree(); i >= 0; --i) {
        Type const coeff = P[i];
        if (coeff == Type(0)) {
            continue;
        }
        if (!firstTerm) {
            oss << (coeff < Type(0) ? " - " : " + ");
        } else if (coeff < Type(0)) {
            oss << '-';
        }
       Type const absCoeff = coeff < Type(0) ? -coeff : coeff;
       if (i == 0 || absCoeff != Type(1)) {
            oss << absCoeff;
            if (i > 0) {
                oss << " * ";
            }
