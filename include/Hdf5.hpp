...
void writeHdf5(std::string filename, T const &h,
	...
	for (int d = 0; d < D; d++) {
		...
		for (IndexTuple<M, D> I = IndexTuple<M, D>::begin(); I != IndexTuple<M, D>::end(); I++) {
	...
	for (int d = 0; d < D; ++d) {
		...
		status = H5Dwrite(dataset_id, H5T_data_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, coordArrays[d]);
	...
	for (int fi = 0; fi < nf; fi++) {
		for (index_type Q = index_type::begin(); Q != index_type::end(); Q++) {
			...
			status = H5Dwrite(dataset_id, H5T_data_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, fieldData[fi][Q.flatIndex()].data());
			status = H5Dclose(dataset_id);
			status = H5Sclose(dataspace_id);
		}
